...
MODEL_SAVE_DIR = 'trained_models_convnet_v2'
MODEL_NAME = 'diabetes_convnet_model_v2.pth'
CONFUSION_MATRIX_NAME = 'convnet_validation_confusion_matrix_v2.png'
TRAINING_METRICS_NAME = 'convnet_training_metrics_v2.png'
FINAL_CONFUSION_MATRIX_NAME = 'convnet_final_test_confusion_matrix_v2.png'
FINAL_CLASSIFICATION_REPORT_NAME = 'convnet_final_classification_report_v2.txt'

# --- Variables Globales para los Nombres de Archivos CSV ---
FINAL_TEST_CSV_NAME = 'diabetes_convnet_prueba_final_v2.csv'
TRAIN_VAL_CSV_NAME = 'diabetes_convnet_entrenamiento_validacion_v2.csv'

# --- Configuración de Pesos de Clase ---
MANUAL_CLASS_WEIGHTS = True
CLASS_WEIGHTS_VALUES = [1.0, 1.5]
...
class DiabetesConvNetV2(nn.Module):
    """
    Red Neuronal Convolucional para clasificación binaria de diabetes.
    La entrada se reorganiza en una matriz 2D de 7x3 para procesamiento convolucional.
    """
    def __init__(self, input_size):
        super(DiabetesConvNetV2, self).__init__()
        self.input_channels = 1
        self.input_height = 7
        self.input_width = 3
        assert input_size == self.input_height * self.input_width, \
            f"El número de características debe ser {self.input_height * self.input_width} para una entrada 7x3."

        self.conv_layers = nn.Sequential(
            nn.Conv2d(self.input_channels, 32, kernel_size=(7, 3), padding=0),
            nn.ReLU(),
            nn.Dropout2d(0.2),
        )

        # Calcular el tamaño de salida después de la convolución
...
def load_and_preprocess_data(batch_size: int = 128):
    ...
    # Reorganizar a 7x3
    assert input_size == 21, "El número de características debe ser 21 para reorganizar a 7x3."
    def reshape_features(X):
        return X.reshape(-1, 21)
    ...
    # Conversión a tensores y reshape
    X_train_tensor = torch.FloatTensor(X_train_balanced).view(-1, 21)
    y_train_tensor = torch.LongTensor(y_train_balanced.values)
    X_val_tensor = torch.FloatTensor(X_val_scaled).view(-1, 21)
    y_val_tensor = torch.LongTensor(y_val.values)
    X_test_tensor = torch.FloatTensor(X_test_scaled).view(-1, 21)
    ...
    val_loader = DataLoader(dataset=val_dataset, batch_size=batch_size, shuffle=False,
                            pin_memory=(device.type == 'cuda'))
    return train_loader, val_loader, X_test_tensor, y_test_tensor, 21, class_weights_tensor
    ...
    for epoch in range(epochs):
        ...
        if val_f1 > best_f1_diabetes:
            ...
            if epochs_no_improve >= early_stopping_patience:
                print(f"Early stopping activado después de {early_stopping_patience} épocas sin mejora en Val F1-Score.")
                break
...
def main():
    try:
        ...
        model = DiabetesConvNetV2(input_size).to(device)
        print("\nIniciando entrenamiento del modelo convolucional V2...")
        ...
        print(f"Gráfico de métricas guardado como: {os.path.join(MODEL_SAVE_DIR, TRAINING_METRICS_NAME)}")
        ...
        print(f"Matriz de Confusión guardada como: {os.path.join(MODEL_SAVE_DIR, CONFUSION_MATRIX_NAME)}")
        ...
        print(f"Matriz de Confusión final guardada como: {os.path.join(MODEL_SAVE_DIR, FINAL_CONFUSION_MATRIX_NAME)}")
        print("\nProceso finalizado.")
