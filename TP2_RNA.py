...
from torch.optim.lr_scheduler import ReduceLROnPlateau  # Corrected import
import os
...
MODEL_NAME = 'diabetes_classifier_model.pth'  # Nombre del archivo del modelo
CONFUSION_MATRIX_NAME = 'confusion_matrix.png'  # Nombre del archivo de la matriz de confusión para validación
TRAINING_METRICS_NAME = 'training_metrics.png'  # Nombre del archivo del gráfico de métricas para validación
FINAL_CONFUSION_MATRIX_NAME = 'final_test_confusion_matrix.png'  # Nombre del archivo de la matriz de confusión para prueba final
FINAL_CLASSIFICATION_REPORT_NAME = 'final_classification_report.txt'  # Nombre del archivo del reporte de clasificación para prueba final
FINAL_METRICS_CHART_NAME = 'final_test_metrics_chart.png'  # Nombre del archivo del gráfico de métricas para prueba final (no usado directamente para un gráfico de época)

# --- Variables Globales para los Nombres de Archivos CSV ---
FINAL_TEST_CSV_NAME = 'diabetes_prueba_final.csv'
TRAIN_VAL_CSV_NAME = 'diabetes_entrenamiento_validacion.csv'

# --- Configuración de Pesos de Clase ---
MANUAL_CLASS_WEIGHTS = True
CLASS_WEIGHTS_VALUES = [1.0, 1.5]  # le damos mas peso a diabetes
...
def load_and_preprocess_data(batch_size: int = 64):
    ...
    # Ya se llamó a ensure_directories_exist en main(), así que el directorio ya debería existir
...
def main():
    try:
        ...
        plot_confusion_matrix(last_val_targets, last_val_preds, class_names=CLASS_NAMES,
                              save_dir=MODEL_SAVE_DIR, file_name=CONFUSION_MATRIX_NAME) # Using CONFUSION_MATRIX_NAME for validation
        print(
