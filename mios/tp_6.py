...
CLASS_WEIGHTS_VALUES = [1.0, 1.5] # ¡Usa los valores que te dieron el mejor resultado!

# --- Configuración de Umbral de Decisión (NUEVO) ---
# Puedes ajustar este umbral para sintonizar el balance Precisión/Recall
# Un valor más alto reduce los falsos positivos (mayor precisión para 'Diabetes')
# pero puede reducir el recall. El valor por defecto implícito es 0.5.
PREDICTION_THRESHOLD = 0.20 # Probamos con con 0.20, 0.40, 0.50, 0.55, 0.60

# Set random seed for reproducibility
torch.manual_seed(SEED)
np.random.seed(SEED)

# --- CUDA Setup --- Por ahora no funciona, puede ser la version de torch.
...
class DiabetesClassifier(nn.Module):
    ...
    def __init__(self, input_size):
        ...
        self.model = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(0.2),
            nn.Linear(8, 2) # Salida para 2 clases (0 y 1)
        )
...
def load_and_preprocess_data(batch_size: int = 64):
    ...
    # Ignoramos prediabetes por el bajo número de casos en el dataset
...
# NOTA: Se ha añadido el parámetro 'threshold' aquí
...
def train_model(model: nn.Module, train_loader: DataLoader, X_test_tensor: torch.Tensor,
    ...
    for epoch in range(epochs):
        ...
        # Evaluación en el conjunto de prueba (con umbral por defecto 0.5 para entrenamiento)
        _, accuracy, f1_diabetes, precision_diabetes, recall_diabetes, _ = \
            evaluate_model(model, X_test_tensor, y_test_tensor, device, threshold=0.5) # Usamos 0.5 para monitoreo interno

        test_accuracies.append(accuracy)
        test_f1_scores_diabetes.append(f1_diabetes)

        print(f"\nEpoch [{epoch + 1}/{epochs}], Avg Train Loss: {epoch_loss:.4f}, "
              f"Test Acc (thr 0.5): {accuracy:.4f}, Test Prec (D, thr 0.5): {precision_diabetes:.4f}, "
              f"Test Recall (D, thr 0.5): {recall_diabetes:.4f}, Test F1 (D, thr 0.5): {f1_diabetes:.4f}")
        ...
        if f1_diabetes > best_f1_diabetes:
            ...
            epochs_no_improve = 0
            print(f"→ Nuevo mejor modelo (F1-Score Diabetes, thr 0.5): {f1_diabetes:.4f}")
        else:
            epochs_no_improve += 1
            if epochs_no_improve >= early_stopping_patience:
                print(f"Early stopping activado después de {early_stopping_patience} épocas sin mejora en F1-Score (Diabetes, thr 0.5).")
                break # Sale del bucle de entrenamiento

    if best_model_state is not None:
        model.load_state_dict({k: v.to(device) for k, v in best_model_state.items()})
        print(f"\nEntrenamiento finalizado. Mejor F1-Score (Diabetes, thr 0.5) en test alcanzado: {best_f1_diabetes:.4f}")
    else:
